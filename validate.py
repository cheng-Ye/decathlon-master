...
def compute_validation_outputs(model, val_iter, field, optional_names=[]):
    ...
    for batch_idx, batch in enumerate(val_iter):
        l, p = model(batch)
        loss.append(l)
        predictions.append(pad(p, 150, dim=-1, val=1))
        a = None
        if hasattr(batch, 'wikisql_id'):
            ...
            a =  pad(batch.answer.data.cpu(), 150, dim=-1, val=1)
        answers.append(a)
    ...
    return loss, predictions, answers, [torch.cat([pad(x, 150, dim=-1, val=1) for x in output], 0) for output in outputs]
...
def validate(task, val_iter, model,  field, world_size, rank, num_print=10, args=None):
    model.eval()
