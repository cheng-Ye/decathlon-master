...
def positional_encodings_like(x, t=None):
    '''
     功能： 计算Aproj positional encoding
    '''
    if t is None:
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, layer, d_model, dropout_ratio):
        super().__init__()
        self.layer = layer                  #layer=MultiHead
        self.dropout = nn.Dropout(dropout_ratio)
        self.layernorm = LayerNorm(d_model)

    def forward(self, *x, padding=None):
        #print('22222222222222222222',x[0])
...
class MultiHead(nn.Module):
    ...
    def forward(self, query, key, value, padding=None):   
        '''decoding阶段，第一步,query=key=value=x=answer_ebeding
        '''                           
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, x, encoding, context_padding=None, positional_encodings=True, answer_padding=None):            #x=answer_embeding,encoding=Cfin
        if positional_encodings:
            x = x + positional_encodings_like(x)
        x = self.dropout(x)
        #print('positional_encodings_like(x) is Ok')
...
def mask(targets, out, squash=True, pad_idx=1):
    ...
    #print('=================targets_after================',targets_after)
...
class Embedding(nn.Module):
    ...
    def __init__(self, vocab_size, trained_dimension, dropout=0.0):

        super().__init__()
        #self.field = field
        dimension = 0
        pretrained_dimension = 400
        self.pretrained_embeddings = [nn.Embedding(vocab_size, pretrained_dimension)]
    ...
    def forward(self, x, lengths=None):
        ...
        #pretrained_embeddings = self.pretrained_embeddings[0](x).detach()
